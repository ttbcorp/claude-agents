...
이후 모든 단계에서 "Codex 설치 여부"를 기억하여 분기한다.
...
Agent(
  ...
  prompt: "당신은 domain-analyst입니다. agents/domain-analyst.md의 지침을 따르세요.
           ...
           디자인 브리프: _workspace/design/01_brief.md (존재하면 UI 요구사항 컨텍스트로 활용)
           산출물 저장 경로: _workspace/01_domain_analysis.md
           파일 첫 줄에 '> Claude 단일 모드 실행 (Codex 미설치)' 명시"
)
```
...
```
Agent(
  description: "TTB 하네스 아키텍처 청사진 작성",
  subagent_type: "codex:codex-rescue",
  model: "opus",
...
```
Agent(
  description: "Claude 아키텍처 청사진 작성",
  subagent_type: "general-purpose",
  model: "opus",
  prompt: "당신은 pattern-analyst입니다. agents/pattern-analyst.md의 지침을 따르세요.
           외부 리서치 결과: _workspace/00_research.md
...
### Step 3: 교차 검증 (Codex 설치 시에만 실행)

**Codex 미설치 시: 이 단계를 완전히 생략한다.** `_workspace/02b_synthesis.md`는 생성하지 않는다.
...
| 상황 | 자체 복구 전략 |
|------|------------|
| Codex 미설치 | Step 1, 2: Claude Opus로 실행. Step 3: 생략 |
