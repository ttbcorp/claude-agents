...
- 코딩 행동 지침: `.claude/skills/ttb-agents/references/coding-guidelines.md`
- 대상 프로젝트 경로
...
1. **코딩 행동 지침** — 파일의 **맨 위**에 위치해야 한다.
   - `skills/agents/references/coding-guidelines.md`를 읽어 전체 내용을 그대로 복사한다.
   - `## 코딩 행동 지침` 섹션 헤더로 시작
...
5. **`## 엔진 설정 (engine.json)`** 섹션 — 아래 표준 블록을 그대로 삽입한다. 사용자가 분석·구현 엔진을 직접 제어할 수 있도록 안내한다:
   ...
   분석·구현 엔진은 `.claude/ttb-agents/engine.json`으로 제어한다. 오케스트레이터가 매 실행마다 이 파일을 읽어 라우팅을 결정한다.

   | engine_mode | 분석 | 구현 |
   |---|---|---|
   | `codex_analysis` (기본) | Codex | Claude Sonnet |
   | `opus_analysis` | Claude Opus | Codex |

   변경하려면 `engine.json`의 `engine_mode` 값을 수정한 뒤 같은 작업을 다시 실행한다.

   **Codex 토큰 소진 시:** 자동으로 Claude-only 모드로 폴백된다 (분석·구현 모두 Claude). 별도 설정 변경 불필요. 복구는 Codex 인증·할당량 갱신 후 새 세션에서.
   ```
