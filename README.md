...
> ## ⚠ DEPRECATED — v2.0부터 `sprote`로 이전합니다
>
> 본 플러그인(`ttb-agents`)은 **v1.18.x를 끝으로 종착**합니다. 후속 plugin은 [`sprote`](https://github.com/ttbcorp/sprote)(라틴어 어원: "너를 위해 존재한다") 입니다.
>
> **sprote v2.0의 주요 변경:**
> - 7개 분리 스킬 → **단일 `sprote` 라우터 + references 모듈** (mode 자동 분기)
> - Codex `harness-cross-validator`·`design-cross-validator`에 **자동 수정 권한** 부여 (workspace-write)
> - 외부 plugin(gstack 등)과의 **bridge 스킬** 신규
>
> **마이그레이션:** Claude Code 마켓플레이스는 plugin name을 unique key로 사용합니다. ttb-agents → sprote는 자동 업데이트되지 않으므로 **수동 재설치**가 필요합니다.
>
> 자세한 마이그레이션 매핑은 v2.0 저장소의 `docs/migration-from-ttb-agents.md`를 참고하세요.
...
Phase 3    build-leader (Sonnet)
            ...
            ├─ validator (Haiku) ── TTB 형식·표준 검증 ──────────────→ 04_validation_report.md
            └─ harness-cross-validator (Codex) ── 청사진 의도·의미 교차 검증 → 05_cross_validation_report.md
```
...
│   ├── setup-permissions.py            # 필수 권한 자동 추가 (멱등성)
│   ├── ensure-engine-json.py           # engine.json 기본값 보장
│   └── ensure-coding-guidelines.py     # CLAUDE.md에 코딩 행동 지침 보장
├── agents/                          # 에이전트 정의 파일
...
│   ├── validator.md                 # 하네스 산출물 TTB 형식·표준 검증 전문가
│   └── harness-cross-validator.md   # Codex 기반 청사진 의도·의미 교차 검증 전문가
└── skills/
    ├── agents/
...
| **build-leader** | Claude Sonnet | 오케스트레이터 | builder 완료 후 docs-keeper + validator + harness-cross-validator를 병렬 실행. engine_mode 수신 후 builder 선택. 내부 에러 자체 복구 |
...
| **validator** | Claude Haiku | build-leader / review | 형식 검증: 구조 무결성·description 품질·트리거 충돌·TTB 표준 준수. PASS/WARN/FAIL 분류 |
| **harness-cross-validator** | Codex (Opus 폴백) | build-leader | 의미 교차 검증: 청사진 의도 부합성·도메인 요구사항 누락·에이전트 관계 일관성·워크플로우 완결성. Claude validator와 병렬 실행하여 두 관점 비교 |
