...
1. Read 도구로 `{CLAUDE_PLUGIN_ROOT}/skills/agents/engine.json` 읽기
2. `{project_root}/.claude/ttb-agents/engine.json` 존재 여부 확인:
   - Read 도구로 해당 경로 읽기 시도 → 성공 시 이미 존재 → 스킵
3. 미존재 → Write 도구로 읽은 전역 engine.json 내용을 `{project_root}/.claude/ttb-agents/engine.json`에 저장
...
1. Read 도구로 `{CLAUDE_PLUGIN_ROOT}/skills/agents/references/coding-guidelines.md` 읽기
