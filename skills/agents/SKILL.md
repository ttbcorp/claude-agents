...
프로젝트 로컬 파일을 우선 확인하고, 없으면 전역 플러그인 기본값으로 폴백한다.

```bash
# 1순위: 프로젝트 로컬 설정
LOCAL_ENGINE="{project_root}/.claude/ttb-agents/engine.json"

# 2순위: 전역 플러그인 기본값
GLOBAL_ENGINE="${CLAUDE_PLUGIN_ROOT}/skills/agents/engine.json"

if [ -f "$LOCAL_ENGINE" ]; then
  cat "$LOCAL_ENGINE"   # → ENGINE_SOURCE="local"
else
  cat "$GLOBAL_ENGINE"  # → ENGINE_SOURCE="global"
fi
```
...
⚠️  오케스트레이터 모델 불일치
  ...
  2. 또는 engine.json의 orchestrator_model을 현재 모델로 변경:
     {ENGINE_SOURCE가 local이면 → {project_root}/.claude/ttb-agents/engine.json}
     {ENGINE_SOURCE가 global이면 → ${CLAUDE_PLUGIN_ROOT}/skills/agents/engine.json}
...
`[local]` — 프로젝트 로컬 `{project_root}/.claude/ttb-agents/engine.json` 사용 중
`[global]` — 전역 기본값 `${CLAUDE_PLUGIN_ROOT}/skills/agents/engine.json` 사용 중
